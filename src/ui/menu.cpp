...
GameState* Menu::moveAll(float dt)
{
    ...
    {
        if (appInput.key(SDL_SCANCODE_UP) || appInput.key(gameinf.getKeys(AppData::PLAYER1).getLeft()))
        {
            if (!upPressed)
            {
                selectedOption--;
        ...
        if (appInput.key(SDL_SCANCODE_DOWN) || appInput.key(gameinf.getKeys(AppData::PLAYER1).getRight()))
        {
            if (!downPressed)
            {
                selectedOption++;
